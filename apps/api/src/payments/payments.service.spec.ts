...
describe("PaymentsService", () => {
  describe("createCheckoutSession()", () => {
    it("creates Stripe session and attaches it to booking", async () => {
      ...
      const service = new PaymentsService(repo, gateway);
      ...
      expect(gateway.createCheckoutSession).toHaveBeenCalledWith(
        expect.objectContaining({ bookingId: BOOKING_ID, amountCents: 2000 }), // 20 * 100
      );
    ...
    it("throws NotFoundException when booking does not exist", async () => {
      ...
      const service = new PaymentsService(repo, makeGateway());
    ...
    it("throws ForbiddenException when requesting user is not the owner", async () => {
      const service = new PaymentsService(makeRepo(), makeGateway());
    ...
    it("uses checkoutAmount from repository for gateway call", async () => {
      const repo = makeRepo({
        findBookingForCheckout: vi.fn().mockResolvedValue({
          id: BOOKING_ID,
          userId: USER_ID,
          checkoutAmount: 100, // $100.00
        }),
      });
      const gateway = makeGateway();
      const service = new PaymentsService(repo, gateway);
      ...
      expect(gateway.createCheckoutSession).toHaveBeenCalledWith(
        expect.objectContaining({ amountCents: 10000 }), // 100 * 100
      );
    });
  });
