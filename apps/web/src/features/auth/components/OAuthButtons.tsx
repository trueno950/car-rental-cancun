...
export async function OAuthButtons({ locale }: OAuthButtonsProps) {
  const env = getWebEnv();
      ...
      <div className="space-y-3">
        {hasGoogle && (
          <form
            action={async () => {
              "use server";
              await signIn("google", { redirectTo: `/${locale}/vehicles` });
            }}
          >
            <Button
              variant="outline"
        ...
        {hasFacebook && (
          <form
            action={async () => {
              "use server";
              await signIn("facebook", { redirectTo: `/${locale}/vehicles` });
            }}
          >
            <Button
              variant="outline"
